...
public class FallbackManipulations implements Manipulations {
    ...
    public ResponseTypes.Result setLocationDetail(final LocationDetail locationDetail) {
        ...
        final var interactionResult = interactionFactory.createUserInteraction(new FilterInputStream(System.in) {
            @Override
            public void close() {
            }
    ...
    public List<String> getRemovableDescriptors() {
        ...
        final var data = interactionFactory.createUserInteraction(new FilterInputStream(System.in) {
            @Override
            public void close() {
            }
    ...
    public ResponseTypes.Result setAlertActivation(final String handle, final AlertActivation activationState) {
        final var interactionMessage = String.format(
            "Set activation state for handle %s to %s",
            handle,
    ...
    public ResponseTypes.Result setAlertConditionPresence(final String handle, final boolean presence) {
        final var interactionMessage = String.format(
            "Set the presence attribute for handle %s to %s",
            handle,
            presence
        );
        final var interactionResult = interactionFactory.createUserInteraction(new FilterInputStream(System.in) {
    ...
    public ResponseTypes.Result setMetricQualityValidity(final String handle, final MeasurementValidity validity) {
        final var interactionMessage = String.format(
            "Set metric quality validity for handle %s to %s",
            handle,
            validity.name()
        );
        final var interactionResult = interactionFactory.createUserInteraction(new FilterInputStream(System.in) {
    ...
    public ResponseTypes.Result setMetricStatus(final String handle, final MetricCategory category,
        ...
        final var interactionMessage = String.format(
            metricStatusString, handle, category
        );
        final var interactionResult = interactionFactory.createUserInteraction(new FilterInputStream(System.in) {
            @Override
            public void close() {
    ...
    public ResponseTypes.Result triggerReport(final QName reportType) {
        ...
        final var interactionMessage = String.format(
            triggerReportString, reportType
        );
        final var interactionResult = interactionFactory.createUserInteraction(new FilterInputStream(System.in) {
            @Override
            public void close() {
    ...
    private String getMetricStatus(final ComponentActivation activation) {
        ...
        switch (activation) {
            ...
            case OFF:
                ...
                break;
            case FAIL:
                metricStatusString = "The measurement/setting for the metric with handle %s and category %s has"
                    + " failed.";
                break;
            default:
