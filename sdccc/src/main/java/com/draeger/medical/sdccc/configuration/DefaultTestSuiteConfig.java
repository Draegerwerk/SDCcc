...
public class DefaultTestSuiteConfig extends AbstractConfigurationModule {
    ...
    void configureTestSuite() {
        bind(TestSuiteConfig.CI_MODE,
            Boolean.class,
            false);

        bind(TestSuiteConfig.GRAPHICAL_POPUPS,
            Boolean.class,
            true);
    }

    void configureTLS() {
        bind(TestSuiteConfig.FILE_DIRECTORY,
            String.class,
            "");
        bind(TestSuiteConfig.KEY_STORE_PASSWORD,
            String.class,
            "");
        bind(TestSuiteConfig.TRUST_STORE_PASSWORD,
            String.class,
            "");
        bind(TestSuiteConfig.PARTICIPANT_PRIVATE_PASSWORD,
            String.class,
            "");
        bind(TestSuiteConfig.TLS_ENABLED_PROTOCOLS,
            String[].class,
            new String[]{"TLSv1.2", "TLSv1.3"});
    }

    void configureNetwork() {
        bind(TestSuiteConfig.NETWORK_INTERFACE_ADDRESS,
            String.class,
            "127.0.0.1");
    }

    void configureProvider() {
        bind(TestSuiteConfig.PROVIDER_ENABLE,
            Boolean.class, false);
    }

    void configureGRpc() {
        bind(TestSuiteConfig.GRPC_SERVER_ADDRESS,
            String.class, "localhost:50051");
    }
