...
public class InvariantSdcParticipantModelBindingTest extends InjectorTestBase {
    ...
    void testRequirementR0080() throws NoTestData {
        ...
        for (String sequenceId : sequenceIds) {
            try (final MdibHistorian.HistorianResult history = mdibHistorian.episodicReportBasedHistory(sequenceId)) {
                ...
                while (first != null) {
                    acceptableSequenceSeen.compareAndSet(false,
                        checkForTypeAttribute(first.findEntitiesByType(AbstractComplexDeviceComponentDescriptor.class),
                            AbstractComplexDeviceComponentDescriptor.class));
                    acceptableSequenceSeen.compareAndSet(false,
                        checkForTypeAttribute(first.findEntitiesByType(ChannelDescriptor.class),
                            ChannelDescriptor.class));
                    acceptableSequenceSeen.compareAndSet(false,
                        checkForTypeAttribute(first.findEntitiesByType(AbstractOperationDescriptor.class),
                            AbstractOperationDescriptor.class));
                    acceptableSequenceSeen.compareAndSet(false,
                        checkForTypeAttribute(first.findEntitiesByType(AlertConditionDescriptor.class),
                            AlertConditionDescriptor.class));
                    acceptableSequenceSeen.compareAndSet(false,
                        checkForTypeAttribute(first.findEntitiesByType(AbstractMetricDescriptor.class),
    ...
    private boolean checkForTypeAttribute(final Collection<MdibEntity> entities,
                                          final Class<? extends AbstractDescriptor> descClass) {
        for (var entity : entities) {
            final var descriptor = entity.getDescriptor(descClass);
            assertNotNull(descriptor.orElseThrow().getType(),
                String.format("No Type attribute provided for %s", descriptor.orElseThrow().getHandle()));
        }
