...
public class DirectMessageModelAnnexTest extends InjectorTestBase {
    ...
    void testRequirementC55() throws NoTestData {
        ...
        final var mdsWithChild =
                mdsEntities.stream().filter(mds -> mds.getChildren().size() > 0).findFirst();
        ...
        sendGetMdDescriptionAndCheckResult(
                getChildrenOfChildren(remoteMdibAccess, children),
                List.of(mdsWithChild.orElseThrow().getHandle()));
        final var entities = remoteMdibAccess.findEntitiesByType(AbstractDescriptor.class);
        final var entitiesHandleList =
                entities.stream().map(MdibEntity::getHandle).collect(Collectors.toList());
